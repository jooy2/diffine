...
- **The wipe has a grip.** The grip was drawn inside the line, and took the line's width of two pixels, so the handle was a slightly thicker stretch of the line. It is now a round grip on a one-pixel line, as in the React package, with a ring round it when the keyboard is on it.
...
- **`diffImages` compares several pictures at once, and `imagesSimilarity` says how alike they are.** Three renderings of one screen, four exports of one asset, a saved version against the last five runs. Each picture is compared with a baseline exactly as `diffImage` would compare it, and the mask is a bit a picture rather than a kind — so `mask[pixel] != 0` is "does anything disagree here" and `mask[pixel] & (1 << i)` is "does this one". A byte holds eight, which is `kMostPictures`.
