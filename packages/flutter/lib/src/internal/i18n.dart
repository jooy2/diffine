/// The words the widgets put on the screen.
///
...
const Map<DiffineLocale, DiffineStrings> _locales = <DiffineLocale, DiffineStrings>{
  DiffineLocale.en: DiffineStrings(
    before: 'Before',
    after: 'After',
...
/// The words for a locale, with whatever the application replaced put over
/// them.
DiffineStrings stringsFor(DiffineLocale locale, DiffineStrings? overrides) {
  return overrides ?? _locales[locale] ?? _locales[DiffineLocale.en]!;
}

/// The words for a locale, before anything is put over them.
DiffineStrings baseStringsFor(DiffineLocale locale) {
  return _locales[locale] ?? _locales[DiffineLocale.en]!;
}
...
String fill(String template, Map<String, Object> values) {
  return template.replaceAllMapped(_placeholder, (Match found) {
    final String name = found[1]!;

    return values.containsKey(name) ? '${values[name]}' : found[0]!;
  });
}
