 ...
 *
 * `tsc` compiles TypeScript and copies nothing else, so a stylesheet would
 * simply not be in the published package. This copies every non-TypeScript file
 * under `src/` to the same path under `dist/`, which is what the `exports` map
 * already points at, and runs the stylesheets through esbuild on the way.
 *
 * `src/styles.css` carries a good deal of prose — why a rule is scoped the way
 * it is, which of two specificities wins, what a custom property is for — and
 * every one of those words is written for somebody reading the source. A reader
 * of a page with a viewer on it should not pay for comments they cannot see.
 *
 * Which makes two things worth checking rather than assuming, and both checks
 * are at the bottom. Every `--diffine-*` custom property the source declares is
...
const declaredIn = (css) => new Set(css.match(/--diffine-[\w-]+(?=\s*:)/g) ?? []);
