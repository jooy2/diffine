...
- **`ImageDiff` takes a list of pictures, not only a pair.** `pictures` is several rather than two, and passing it is what turns the list on. Each gets a pane of its own with what _that_ picture disagrees with the baseline about marked on it; the pane holding the baseline is marked with everywhere anything disagrees, because the baseline disagrees with nothing and a pane with nothing on it reads as a pane nobody looked at. `baseline` names which one, `picturesResult` takes a comparison already worked out, and `onPicturesDiff` reports one.

  Everything else works as it did: one zoom and one position across every pane, the boxes and the buttons that step through them, the loupe showing every picture at the point under the pointer, and the bar above and below cut into as many parts as there are panes — with the controls in a row of their own past two, since a title with five names in it has no room for buttons. `overlay` and `wipe` are a question about two pictures and fall back to `split` for more than two.
