 ...
 *
 * A package keeps its changelog beside its manifest, where npm and anyone
 * browsing that package expect to find it. A second copy under `docs/` would be
 * two files that say the same thing until the day one of them does not, so the
 * docs' copy is generated instead — written before VitePress starts and ignored
 * by git.
 *
...
/** Which package's changelog becomes which page. */
