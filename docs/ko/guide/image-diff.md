...
- **상자**는 그 픽셀이 뭉친 곳마다 둘립니다. 전체를 창에 맞춰 놓으면 세 픽셀짜리 변경은 보이지 않지만 상자는 남습니다. 밝은 선 위에 어두운 선을 겹쳐 그리므로 아래 사진이 어떤 색이든 보입니다. **파란** 상자는 막대의 버튼으로 지금 짚어 놓은 변경입니다.
...
포인터가 어느 창에 있든 양쪽을 다 보여 줍니다. 나란히 보기에서는 창마다 이미지가 하나씩이고, 이 배율에서 묻는 질문은 한쪽에 대한 것이 아니기 때문입니다. 패널은 포인터에서 가장 먼 모서리에 붙습니다. 포인터 밑에 있는 패널은 확대해서 보려던 것을 가리는 패널이니까요. 손가락이 아니라 포인터를 따라가므로 터치 화면에서는 나타나지 않습니다.
