...
글자와 사선은 선이 실제로 지나는 자리 양옆 픽셀에 색을 나눠 담아 그립니다. 얼마씩 담을지는 렌더러가 알아서 계산하므로, 같은 화면을 브라우저 둘이 그리면 글자마다 값이 다르면서 보이는 것은 같습니다. 어떤 픽셀이 자기 색을 가진 것이 아니라 주변의 섞임이고, 변화의 크기가 그 자리에 원래 있던 밝기 차이를 넘지 않을 때 보정으로 봅니다. 흰 바탕 한가운데서 검게 바뀐 픽셀은 두 조건 모두 통과하지 못합니다.

공짜는 아닙니다. 달라진 픽셀마다 이웃 여덟 개를 다시 읽습니다.
