...
`diffText`는 두 문서 사이에서 무엇이 달라졌는지 계산해 돌려줍니다. React도 DOM도 건드리지 않기 때문에 진입점도 따로 있습니다. 요약 한 줄, 배지의 숫자, 워커에서 돌리는 계산에는 그림이 아니라 답이 필요하니까요.
...
변경 하나는 함께 바뀐 줄들의 묶음입니다. "변경이 몇 건이냐"라고 물을 때 세는 단위가 이것입니다. 안 바뀐 줄들은 목록에 없습니다. 변경과 변경 사이의 빈 자리가 그것입니다.

각 항목은 양쪽에서 어느 줄들을 덮는지와 어느 행을 차지하는지를 들고 있습니다.
...
기본값 `word`가 대개 원하는 결과입니다. 원래 있던 단어와 글자 세 개가 겹친다고 그것까지 짚는 대신 옮겨간 단어를 짚습니다. `character`는 한 단계 더 내려가 자소 하나씩 비교하므로 숫자 가운데 한 자리가 바뀐 경우에 알맞습니다. `none`은 바뀐 줄을 바뀐 줄로만 두고 더 말하지 않습니다.
...
무시한 공백도 화면에는 그대로 그립니다. 어떤 줄을 같다고 볼지만 바뀌고 읽는 사람이 보는 것은 안 바뀝니다. 들여쓰기만 다시 한 파일에 `surrounding`이 쓸모 있는 이유입니다.
...
켜면 엔진이 같다고 판단한 구간이 서로 다른 두 문자열일 수 있습니다. 조각을 한 목록이 아니라 양쪽으로 나눠 돌려주는 이유가 이것입니다. 목록이 하나면 둘 중 하나를 골라야 하는데, 어느 쪽을 고르든 다른 문서에는 없던 글자가 됩니다.

### `inlineThreshold`

줄 안의 단어를 짚어 줄 만큼 두 줄이 닮았는지 판단하는 기준입니다. 0에서 1 사이이고 기본은 `0.3`입니다.

고쳐진 두 줄은 단어 대부분이 겹칩니다. 그냥 마주 보게 된 두 줄은 쉼표 하나와 모음 몇 개가 겹칠 뿐이고, 그걸 짚으면 아무 의미 없는 조각으로 줄이 얼룩덜룩해집니다. 기준 아래로 내려가면 양쪽 모두 그냥 바뀐 줄 하나로 그립니다.
