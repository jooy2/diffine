...
입력란마다 찾기가 따로 있습니다. 뷰어의 창과 똑같이 위쪽 막대에 버튼이 있고 아래에 자기 막대가 열리며, **Ctrl+F**는 커서가 놓인 입력란의 막대를 엽니다. **Ctrl+R**은 바꾸기 줄까지 함께 열어 줍니다.

커서는 찾기를 따라갑니다. 이동한 자리가 곧 입력란의 선택 영역이라서, **Escape**로 막대를 닫으면 커서는 방금 보던 자리에 남고 거기서 이어 칠 수 있습니다.
...
`readOnly`인 쪽에는 바꾸기 줄 없이 찾기만 놓입니다. `search={false}`는 둘과 단축키를 함께 끕니다. **Ctrl+R**이 브라우저의 새로 고침으로 남아야 하는 페이지라면 이 값을 쓰세요.
...
긴 문서에 문법을 돌리는 것이 여기서 한 글자를 칠 때 드는 비용의 대부분입니다. 같은 문서를 비교하는 데 1밀리초가 든다면 색을 입히는 데는 수십 밀리초가 듭니다. 그래서 색은 뒤로 미룹니다. 방금 친 글자는 직전에 만들어 둔 색으로 그리고, 새 색은 다음 키가 눌리면 중단할 수 있는 단계에서 도착합니다. 그 사이에 보이는 것은 지금 치고 있는 줄이 한 박자 전의 색으로 칠해진 모습입니다.
