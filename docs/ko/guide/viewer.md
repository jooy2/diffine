...
도착한 변경은 왼쪽 가장자리에 선으로 표시되고, 두 창 사이의 띠는 선이 굵어집니다. 스크롤이 멈춘 뒤에도 어디에 있는지가 남아 있습니다.
...
`search={false}`로 버튼과 단축키를 함께 끕니다.
...
브라우저 자체의 찾기 기능이 스크롤 밖의 글자까지 닿아야 하는 페이지라면 `virtualize={false}`로 끄세요. 그리지 않은 것은 찾을 수 없습니다. 뷰어에 들어 있는 찾기가 그 문제의 다른 답이고 보통은 이쪽입니다. 페이지가 아니라 문서를 읽으므로 9000번째 줄에 있는 글자도 찾아서 그 자리로 스크롤합니다.
