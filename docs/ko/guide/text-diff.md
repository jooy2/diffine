...
구간이 무엇인지만 이름 붙이고 색은 테마에 맡기고 싶다면 `style` 옆에 `kind`도 있습니다.
...
꾸미는 자리는 `.diffine-slot`과 `.diffine-widget`입니다. 이 두 클래스에는 돌려준 내용 주위의 여백밖에 들어 있지 않습니다.

:::

::: fw flutter

돌려준 것을 그대로 그리고, 그 주위의 여백만 더합니다.
...
`scale`은 다른 것을 정합니다. 글자를 무엇으로 쓸지가 아니라, 컴포넌트 자신의 컨트롤까지 포함해 전체를 얼마나 크게 그릴지입니다.

```tsx
<TextDiff before={saved} after={draft} scale={1.25} />
```

글자, 줄 번호 칸, 막대, 버튼과 아이콘, 메뉴에 모두 같은 수를 곱하고, `font`로 준 크기에도 곱합니다. 상자의 높이는 페이지가 준 그대로입니다. 속으로는 `--diffine-scale` 속성이라서 여러 비교를 감싼 엘리먼트에 스타일시트로 한 번만 지정할 수도 있습니다. 다만 모든 곳에 닿는 것은 prop입니다. 언어 메뉴가 열리는 위치는 컴포넌트가 계산하므로 prop으로 줄 때만 따라옵니다.
...
`scale`은 다른 것을 정합니다. 글자를 무엇으로 쓸지가 아니라, 위젯 자신의 컨트롤까지 포함해 전체를 얼마나 크게 그릴지입니다.

```dart
TextDiff(before: saved, after: draft, scale: 1.25);
```

글자, 줄 번호 칸, 막대, 버튼과 아이콘, 메뉴에 모두 같은 수를 곱하고, `font`로 준 크기에도 곱합니다. `height`와 모서리 반지름은 테마에 있는 그대로입니다.
