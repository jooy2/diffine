...
전체 목록은 [API](../api/#팔레트)에 있습니다. 테마를 주면 `colorScheme`도 함께 정해집니다. 테마 자체가 어느 팔레트인지에 대한 결정이기 때문입니다. 그중 한 필드는 에디터만 씁니다. `selection`은 반투명해야 합니다. 선택 영역 아래의 글자는 입력란 뒤에 그려지므로, 불투명한 색을 깔면 골라 낸 글자가 있던 자리에 사각형만 남습니다.
