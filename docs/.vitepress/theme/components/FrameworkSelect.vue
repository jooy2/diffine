...
<template>
  <div class="diffine-fw-switch">
    <p id="diffine-fw-label" class="diffine-fw-title">{{ label }}</p>
    <div class="diffine-fw-track" role="radiogroup" aria-labelledby="diffine-fw-label">
      <label v-for="item in FRAMEWORKS" :key="item.id" class="diffine-fw-option" :data-fw="item.id">
        <input
          type="radio"
          name="diffine-fw"
          :value="item.id"
